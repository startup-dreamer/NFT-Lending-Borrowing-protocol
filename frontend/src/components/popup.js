  ...
  const Borrow = async () => {
    try {
      if ((borrowAmount && borrowTime !== 0) && (borrowAmount <= BorrowingPower)) {
        ...
        const Tx1 = await approveToken(contract, '0xff0AF63633f2FEeB37a9E6bD46013A6333B20460', tokenId);
      ...
      <div className="lend_popup">
        ...
        <div className="right_popup">
          <div className="lend_section">
            ...
            <button className='lend_bttn_popup' onClick={() => { Lend() }}>{loading ? 'Depositing...' : 'Lend'}</button>
          </div>
        </div>
